...
const CategoriesCard = ({ handleClose }: Props) => {
  ...
  // Map categoryData to the required format
  const categoryData = getCategoryQuery?.data?.data?.data ?? [];
  const formattedCategoryData = categoryData.map((category: any) => ({
    title: category.name, // Assuming backend provides a name field
    items: category.subcategories?.map((sub: any, index: number) => ({
      key: index, // Use index as key or a unique field
      value: sub.name, // Assuming backend provides subcategory name
    })) ?? [],
  }));

  // Helper function to divide items into columns
  const distributeItemsIntoColumns = (
    allItemsWithTitles: { title: string; items: Item[] }[],
    itemsPerColumn: number,
    numColumns: number
    ...
    allItemsWithTitles.forEach(({ title, items }) => {
      ...
      items.forEach((item) => {
        ...
        // Move to the next column if the current one reaches max items
        if (itemCountInColumn === itemsPerColumn) {
          currentColumn++;
          itemCountInColumn = 0;
        }

        // Reset to the first column if the last column is filled
        if (currentColumn === numColumns) {
          currentColumn = 0;
        }
      });
  ...
  // Distribute the formatted categories into columns
  const columns = distributeItemsIntoColumns(
